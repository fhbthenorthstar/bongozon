import { beautyAwardBlogs } from "@/src/utils";
import { format, parseISO } from "date-fns";
import Image from "next/image";
import Link from "next/link";
import SectionHeading from "../utils/section-heading";

const BeautyAwardPosts = ({ blogs }) => {
  const posts = beautyAwardBlogs(blogs);
  return (
    <section className="w-full py-12">
      <SectionHeading heading="BEAUTY AWARDS"/>
      {/* Fix the grid below */}
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 md:gap-10 text-center">
      {posts.map((post, index) => (
        <div key={index} 
        className={`group text-dark dark:text-light ${index === 1 ? "lg:col-span-2 lg:row-span-2 " : ""} ${index === 0 ? "flex flex-col items-center justify-center mt-0 md:mt-12" : ""} ${index === 2 ? "flex flex-col items-center justify-center mt-0 md:mt-12" : ""}`}>
...
export default BeautyAwardPosts;
